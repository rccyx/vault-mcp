...
// node_modules/envyx/dist/index.cjs
var zod = require_lib();
var isObject = (v) => typeof v === "object" && v !== null;
var isZodLike = (s) => isObject(s) && typeof s.safeParse === "function";
var pickMessage = (v) => {
  const m = v?.message;
  return typeof m === "string" && m.trim() ? m : undefined;
};
var extractZodMessages = (err) => {
  const out = [];
  const pushFrom = (arr) => {
    if (!Array.isArray(arr))
      return;
    for (const item of arr)
      out.push(pickMessage(item) ?? "Invalid value");
  };
  pushFrom(err.issues);
  pushFrom(err.errors);
...
function createEnv(options) {
  ...
  for (const key of Object.keys(vars)) {
    const validatorUnknown = vars[key];
...
class HttpVaultClient {
  ...
  sys = {
    addPolicy: async ({ name, policy }) => {
      return this.request("PUT", `sys/policies/acl/${encodeURIComponent(name)}`, { policy });
    },
    listPolicies: async () => {
...
class VaultMcpServer {
  ...
  constructor(vaultAddress, vaultToken) {
    this.server = new McpServer({
      name: "vault-mcp",
      version: "1.0.0",
      description: "MCP server for HashiCorp Vault secret and policy operations"
    });
  ...
  const env = $createEnv({
    vars: {
      ...
      VAULT_TOKEN: z.string().min(3).startsWith("hvs.", {
        message: "VAULT_TOKEN must start with 'hvs.' prefix for HashiCorp Vault tokens"
      }),
